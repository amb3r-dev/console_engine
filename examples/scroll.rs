...
fn main() {
    ...
    // draw the background
    engine.fill(pixel::pxl(' ', None, Some(Color::Cyan), None));
    ...
    engine.fill_rect(
        ...
        pixel::pxl(' ', None, Some(Color::White), None),
    );
    ...
    engine.rect(
        ...
        engine.get_width() as i32 - 5,
        engine.get_height() as i32 - 5,
        pixel::pxl(' ', None, Some(Color::Blue), None),
    );
    ...
    loop {
        ...
        if engine.is_key_held(KeyCode::Left) {
            engine.scroll(-1, 0, pixel::pxl(' ', None, Some(Color::Cyan), None)); // scroll to the right
        }
        if engine.is_key_held(KeyCode::Right) {
            engine.scroll(1, 0, pixel::pxl(' ', None, Some(Color::Cyan), None)); // scroll to the left
        }
        if engine.is_key_held(KeyCode::Up) {
            engine.scroll(0, -1, pixel::pxl(' ', None, Some(Color::Cyan), None)); // scroll to the bottom
        }
        if engine.is_key_held(KeyCode::Down) {
            engine.scroll(0, 1, pixel::pxl(' ', None, Some(Color::Cyan), None)); // scroll to the top
        }
